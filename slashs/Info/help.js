...
function getAll(client, interaction) {
    const helpembed = new Discord.MessageEmbed()
        ...
        /* Admin */.addField("🚫 Admin", "`announce` `channel` `drop` `emojifind` `giveaway` `poll` `reactionrole` `usercounter`", true)
        ...
        /* Fun */.addField("🎲 Fun", "`8ball` `achievement` `activities` `ascii` `coinflip` `ship`", true)
        /* Games */.addField("🎮 Games", "`akinator` `connect4` `flood` `hangman` `match` `rockpaperscissors` `slotmachine` `tictactoe`", true)
        ...
        /* Other */.addField("💡 Other", "`calc` `mcstat` `nick` `percentage` `ping` `qr` `remindme` `today` `translate` `urban` `weather`", true)
