module.exports = (client, member) => {
	...
	if (client.settings.get(member.guild.id, "welcomemessage") === "true" && welcomechannel) welcomechannel.send({embeds:[client.chill.welcomeMessage(member, member.guild)]}).catch(err => {return})
	// role on join
	let roleonjoin = member.guild.roles.cache.find(role => role.id === client.settings.get(member.guild.id, "roleonjoin"));
	if (client.settings.get(member.guild.id, "welcomerole") === "true" && roleonjoin && !member.guild.features.includes('MEMBER_VERIFICATION_GATE_ENABLED')) member.roles.add(roleonjoin).catch(err => {return})
};